...
**DTW**:
* `dtw.windowDuration`: maximum duration (in seconds) of the Sakoe-Chiba window when performing DTW alignment. If the window duration is shorter than about 10-20% of the audio duration, consider increasing this value, for better results. Note: higher values would consume quadratically larger amounts of memory. The estimated memory requirement is shown in the log before alignment starts. Defaults to `120` (two minutes)
* `dtw.granularity`: adjusts the MFCC frame width and hop size based on the profile selected. Can be set to either `auto` (auto-selected based on audio duration and task), `x-low` (200ms width, 80ms hop), `low` (100ms width, 40ms hop), `medium` (50ms width, 20ms hop), `high` (25ms width, 10ms hop), `x-high` (20ms width, 5ms hop). If the main goal is producing subtitles, a lower granularity profile is recommended, since it's faster and requires significantly less memory, enabling longer audio durations to be processed quickly. For example, `low` can align 30 minutes of audio (5 minute window) using ~1.5GB of memory in less than 1 minute, with reasonable accuracy. Defaults to `auto`.
