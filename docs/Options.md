...
* `dtw.granularity`: adjusts the MFCC frame width and hop size based on the profile selected. Can be set to either `high` (25ms width, 10ms hop) , `medium` (50ms width, 25ms hop) or `low` (100ms width, 50ms hop). If the main goal is producing subtitles, a lower granularity profile is recommended, since it's faster and requires significantly less memory, enabling longer audio durations to be processed quickly. For example, `low` can align 30 minutes of audio (5 minute window) using 1GB of memory in less than 1 minute. Defaults to `high`.
